...
class DataLoader:
    ...
    def __init__(self, data):
        self.cfg = {
            'movie': {
                ...
                'kg_path': 'data/movie/KG_1hot.txt',
    ...
    def _build_dataset(self):
        ...
        #c = self.user_encoder.inverse_transform([1])
        #c1 = self.entity_encoder.transform(c)
        # negative sampling
    ...
    # user的新索引转化为旧索引
    def newindex_oldindex_user(self, u_ids):
        return (self.user_encoder.inverse_transform(u_ids))

    # item的新索引转化为旧索引
    def newindex_oldindex_entity(self, v_ids):
        """
        v_ids为int
        """
        item2id_dict = dict(zip(self.df_item2id['id'],self.df_item2id['item']))
        v = item2id_dict[v_ids]
        return v

    # user的旧索引转化为新索引
    def oldindex_newindex_user(self, v_ids):
        v = []
        for i in v_ids:
            v.extend(self.user_encoder.transform([i]))
        return v

    # item的旧索引转化为新索引
    def oldindex_newindex_entity(self, v_ids):
        item2id_dict = dict(zip(self.df_item2id['item'], self.df_item2id['id']))
        v = []
        for i in v_ids:
            v.extend([(item2id_dict[int(i)])])
        return v