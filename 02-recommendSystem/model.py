...
class KGCN(torch.nn.Module):
    ...
    def _gen_adj(self):
        ...
        for e in range(self.num_ent):
            if e in self.kg and len(self.kg[e]) > 0:
                ...
                # For entities without neighbors, use self-loop
                self.adj_ent[e] = torch.LongTensor([e] * self.n_neighbor)
                self.adj_rel[e] = torch.zeros(self.n_neighbor, dtype=torch.long)
    ...
    def forward(self, u, v):
        ...
        u = torch.clamp(u, 0, self.num_user - 1)
        v = torch.clamp(v, 0, self.num_ent - 1)
    ...
    def _get_neighbors(self, v):
        ...
        for h in range(self.n_iter):
            # Clamp entity indices to valid range before accessing adjacency matrix
            current_entities = torch.clamp(entities[h], 0, self.num_ent - 1)
            neighbor_entities = torch.LongTensor(self.adj_ent[current_entities]).view((self.batch_size, -1)).to(self.device)
            neighbor_relations = torch.LongTensor(self.adj_rel[current_entities]).view((self.batch_size, -1)).to(self.device)
    ...
    def _aggregate(self, user_embeddings, entities, relations):
        ...
        '''
        # Ensure all entity indices are within valid range
        entity_vectors = [self.ent(torch.clamp(entity, 0, self.num_ent - 1)) for entity in entities]
        # Ensure all relation indices are within valid range
        relation_vectors = [self.rel(torch.clamp(relation, 0, self.num_rel - 1)) for relation in relations]
